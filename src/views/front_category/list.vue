<template>
  <div class="app-container">
    <el-header>
      <el-row>
        <el-col :span="2">
          <router-link :to="{ path: '/front_category/add' }">
    ...
    >
      ...
      <el-table-column align="center" label="操作" width="220px">
        <template slot-scope="scope">
          <router-link :to="{ name: '前台分类保存', query: { categoryId : scope.row.categoryId }}">
...
<script>
import { frontCategoryList, deleteFrontCategory } from '@/api/category'
  ...
  methods: {
    deleteCategory(id, index) {
      this.$confirm('此操作将永久删除该分类, 是否继续?', '提示', {
        ...
        deleteFrontCategory({ categoryId: id }).then(response => {
          this.fetchData()
          this.$message({
            type: 'success',
            message: '删除成功!'
